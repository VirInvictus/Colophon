...
- [x] **Verify the Flatpak build.** *(Done 2026-07-23.)*
      `org.freedesktop.Sdk.Extension.rust-stable//25.08` installed (556 MB
      down / 2.0 GB on disk), then `flatpak-builder` run against the GNOME 50
      manifest: **build succeeded and the built binary runs under the
      runtime** (`flatpak-builder --run ... colophon --help` exits 0). The
      bump is now verified, not merely reasoned about.

      **One manifest bug found and fixed in the process:** the build had never
      worked, independently of the runtime version. `flatpak-builder`
      sandboxes builds without network, and this manifest carries no vendored
      cargo sources, so cargo could not resolve `index.crates.io` and the
      build died at the first dependency. Added `build-args: --share=network`
      to `build-options`, matching what Atrium already does and documents.
      **This is a local-developer convenience, not Flathub-ready:** Flathub
      forbids network access during build, so shipping there still needs
      vendored cargo sources generated by `flatpak-cargo-generator.py`.
