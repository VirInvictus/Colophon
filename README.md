...
**Status:** v2.4.1. Ingestion, the app shell, the full widget catalogue,
...
hard-edged, and the whole stylesheet is generated by the app. 2.1.0 added
two overview charts from the statistics database alone: speed by hour of
...
hardened the import path: a corrupt database can no longer crash the app,
and a failed import can never touch the good snapshot. See
...
bookmarks**, drawn as markers at their true place on the activity strip and
listed with their full text on the book page. A
...
By default a reading day is the calendar day. If you regularly read past
midnight, set the minute your reading day starts in Preferences
(Ctrl+comma): reading done in the small hours before it counts toward the
previous day, so one late-night sitting reads as one day in totals,
streaks, heatmaps, and time windows. The change applies live, and clock
hours never move: the when-do-I-read heatmap still shows the real hour you
read.
